...
def team_pitching(kw, pit_df):
    ...
    if t.empty:
        return {}
    ip   = num(t["IP"]).fillna(0)
    era  = num(t["ERA"]).fillna(0)
    whip = num(t["WHIP"]).fillna(0)
    so   = num(t["SO"]).fillna(0)
    bb   = num(t["BB"]).fillna(0)
    hr   = num(t["HR"]).fillna(0)
    tip  = ip.sum()
    if tip == 0:
        return {}
    return {
...
def pitcher_stats(name, pit_df):
    ...
    for col, fmt in [("ERA",":.2f"),("WHIP",":.3f"),("SO",":.0f"),("BB",":.0f"),("IP",":.1f"),("HR",":.0f")]:
        try:
            out[col] = format(float(r[col]), fmt[1:])
        except:
            pass
    return out
...
def api_games():
    try:
        r = requests.get(
            "https://api.the-odds-api.com/v4/sports/baseball_mlb/odds",
            params={"apiKey": ODDS_API_KEY, "regions": "us", "markets": "h2h", "oddsFormat": "american", "daysFrom": 1},
            timeout=10,
        )
        if r.status_code != 200:
...
def api_predict():
    ...
    def fmt_stats(s, name):
        ...
        sp = s.get("sp", {})
        if sp:
            lines.append(f"  SP ({sp.get('Name','')}): ERA {sp.get('ERA','?')} WHIP {sp.get('WHIP','?')}")
        return "\n".join(lines)

    odds_str = ""
    if odds.get("home_ml"):
        odds_str = (
            f"LIVE ODDS ({odds.get('bookmaker','Sportsbook')}): "
